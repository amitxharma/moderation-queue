...
const PreviewModal = () => {
  ...
  const currentIndex = posts.findIndex(
    (p) => p.id === selectedPostForPreview.id
  );
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-6xl w-full h-[90vh] sm:h-[80vh] overflow-hidden transition-colors duration-300 flex flex-col"
        >
          {/* Header */}
          ...
          <div className="flex-1 flex flex-col sm:flex-row overflow-hidden">
            {/* Content Section */}
            <div className="w-full sm:w-1/2 p-4 overflow-y-auto order-1 sm:order-none">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <div className="flex items-center gap-1">
                  <Clock size={12} />
                  <span>
                    {formatTimeAgo(selectedPostForPreview.reportedAt)}
                  </span>
                </div>
              ...
              {selectedPostForPreview.rejectionReason && (
                <div className="mt-3 p-2 bg-red-50 border border-red-200 rounded">
                  <h3 className="font-semibold text-red-800 text-xs mb-1">
                    Rejection Reason:
                  </h3>
            ...
            {/* Image Section */}
            <div className="w-full sm:w-1/2 p-4 border-t sm:border-t-0 sm:border-l border-gray-200 dark:border-gray-700 order-2 sm:order-none">
              {selectedPostForPreview.imageUrl ? (
                <img
                  src={selectedPostForPreview.imageUrl}
                  alt="Post content"
                  className="w-full h-64 sm:h-full object-contain rounded-lg"
                />
              ) : (
                <div className="w-full h-64 sm:h-full bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center">
